...
TESTS = economics_test block_format_test compact_block_relay_test p2p_protocol_test p2p_transport_test peer_manager_test peer_store_test peer_event_loop_test block_transfer_test archive_retrieval_test stage7_relay_test stage7_node_admission_test stage7_transaction_frame_test header_sync_test download_coordinator_test mini_node_history_test block_validation_test chain_index_test difficulty_test block_store_test active_chain_test local_node_test private_admission_test onuros_privacy_engine_backend_test onuros_tracked_witness_v2_test persistent_shielded_state_test private_reward_test private_mempool_test private_block_test private_node_commit_test pruning_test state_snapshot_test snapshot_trust_test block_production_policy_test

.PHONY: all node network-node tls-probe mining-endpoint benchmark bandwidth-benchmark tls-test test kawpow-test mining-endpoint-test sanitize clean
...
test: $(addprefix build/,$(TESTS))
	@for test in $(TESTS); do chmod 755 ./build/$$test && ./build/$$test || exit $$?; done
	...
	@bash scripts/audit-active-privacy-protocol.sh
...
sanitize:
	...
	@for test in $(TESTS); do \
	  ...
	  if [ "$$name" = "snapshot_trust_test.cpp" ] || \
...
clean:
	$(RM) $(addprefix build/,$(TESTS)) build/onuros-local-node build/onuros-stage7-network-node build/onuros-stage7-tls-probe build/onuros-mining-endpoint build/onuros-private-tps-benchmark build/onuros-stage7-bandwidth-benchmark
