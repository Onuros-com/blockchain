...
[[ "$role" == server || "$role" == client ]] || {
  echo "invalid synchronization role: $role" >&2
  exit 2
}
...
grep -qx "role=$role" "$node_manifest"
grep -qx 'synchronization=PASS' "$node_manifest"
grep -qx 'process_exit_status=0' "$node_manifest"
grep -qx 'private_payloads_logged=false' "$node_manifest"
grep -q "^stage7_sync=PASS role=$role " "$node_log"
...
cp "$node_log" "$output/node.log"
{
  ...
  printf 'stage7_sync_evidence=PASS\n'
